...
WAL_COLORS_PATH = Path('~/.cache/wal/colors.json')
BACKUP_COLORS_PATH = Path('~/.config/qtile/colors.json')
COLORS_PATH = None 

if WAL_COLORS_PATH.expanduser().exists():
    COLORS_PATH = WAL_COLORS_PATH
else:
    COLORS_PATH = BACKUP_COLORS_PATH

SystemTheme = json.load(open(COLORS_PATH.expanduser(), 'r'))
SystemTheme['normal'] = []
SystemTheme['bright'] = []

for i in range (0, 8):
    SystemTheme['normal'].append(SystemTheme['colors'][f'color{i}'])

for i in range (8, 16):
    SystemTheme['bright'].append(SystemTheme['colors'][f'color{i}'])

CURRENT_SCREEN_BORDER =  "#ff6c6b"
